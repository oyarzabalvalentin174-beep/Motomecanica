...
export default function ModalPagoVenta({ open, onClose, cart, subtotal, onSaleSuccess }) {
  ...
  const btnBase =
    "rounded-xl px-4 py-3 text-sm font-semibold transition focus:outline-none focus:ring-2 focus:ring-offset-2";
  ...
  return (
    ...
    >
      <div className="max-h-[90vh] w-full max-w-md overflow-y-auto rounded-2xl border border-zinc-200 bg-white p-6 shadow-xl">
        <h2 id="modal-pago-titulo" className="text-lg font-semibold text-zinc-900">
        ...
        {step === "efectivo" ? (
          <div className="mt-5 space-y-4">
            <p className="text-sm text-zinc-600">
              En efectivo aplicamos <span className="font-semibold text-zinc-900">10% de descuento</span> sobre el
              subtotal del carrito.
            </p>
            <dl className="space-y-1 rounded-xl border border-zinc-100 bg-zinc-50/80 px-4 py-3 text-sm">
              <div className="flex justify-between text-zinc-600">
                <dt>Subtotal</dt>
                <dd className="font-medium text-zinc-900">{money(subtotal)}</dd>
              </div>
              <div className="flex justify-between text-zinc-600">
                <dt>Descuento 10%</dt>
                <dd className="font-medium text-emerald-700">−{money(descEfectivo)}</dd>
              </div>
              <div className="flex justify-between border-t border-zinc-200 pt-2 text-base font-semibold text-zinc-900">
                <dt>Total a pagar</dt>
                <dd>{money(totalEfectivo)}</dd>
              </div>
            </dl>
            <label className="block text-xs font-semibold uppercase tracking-wide text-zinc-500">
              ...
              <input
                ...
                className="mt-1 w-full rounded-xl border border-zinc-200 bg-white px-3 py-2.5 text-[15px] text-zinc-900"
                placeholder="0,00"
                autoComplete="off"
              />
            </label>
            <p className="text-sm font-medium text-zinc-800">
              Vuelto:{" "}
              <span className="text-lg text-emerald-700">{vuelto != null ? money(vuelto) : "—"}</span>
            </p>
            <div className="flex flex-wrap gap-2 pt-2">
              ...
              <button
                ...
                onClick={async () => {
                  if (!Number.isFinite(montoNum) || montoNum < totalEfectivo) {
                  ...
                  setSaleSnapshot({
                    ...
                    vuelto: round2(montoNum - totalEfectivo),
        ...
        {step === "transferencia" ? (
          <div className="mt-5 space-y-4">
            <p className="text-sm text-zinc-600">
              Enviá el importe de <span className="font-semibold text-zinc-900">{money(totalTarjetaTransfer)}</span> al
              siguiente alias:
            </p>
        ...
        {step === "tarjeta" ? (
          <div className="mt-5 space-y-4">
            ...
            <p className="rounded-xl border border-violet-200 bg-violet-50 px-4 py-4 text-center text-2xl font-bold text-violet-950">
