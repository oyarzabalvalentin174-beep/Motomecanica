...
import { getReportePreview } from "@/lib/reportesExcel";
...
export async function GET(request) {
  ...
  try {
    const data = await getReportePreview(tipo, {
    ...
    return NextResponse.json({ status: "ok", tipo, ...data });
  } catch (e) {
    return NextResponse.json({ error: e?.message || "Error al cargar datos" }, { status: 500 });
  }
}
