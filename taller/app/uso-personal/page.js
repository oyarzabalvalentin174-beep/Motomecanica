...
import UsoPersonalClient from "@/components/UsoPersonalClient";
...
export default async function UsoPersonalPage() {
  await requireSession("/uso-personal");
  let productos = [];
  let listError = null;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-zinc-200/90 via-zinc-100 to-zinc-200/80">
      ...
      <main className="min-h-screen pt-24 sm:pt-28">
        <UsoPersonalClient initialProductos={productos} listError={listError} />
      </main>
    </div>
  );
}
